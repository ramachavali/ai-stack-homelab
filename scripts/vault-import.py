...
def parse_args(project_root: Path) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--path",
        default=os.environ.get("KV_PATH", "ai-stack/env"),
        help="KV path inside mount (default: ai-stack/env)",
    )
    parser.add_argument(
        "--vault-addr",
...
def resolve_vault_token(project_root: Path, explicit_token: str | None) -> str:
    ...
    init_file = project_root / "../coreservices-homelab/secrets/vault-init.json"
    if init_file.exists():
        try:
...
def ensure_container_running(container_name: str) -> None:
    ...
    if container_name not in names:
        ...
        print("Start core services first: cd ../coreservices-homelab && ./scripts/start.sh", file=sys.stderr)
        sys.exit(1)
